...
namespace _5GAutoTool
{
    public class N5182B
    {
        ...
        public string Alias;
        public string Name = "N5182B";
        public double TriggerDelay = 0;
        //public Keysight.SignalStudio.N7631.Api api;
        string currLoadPath = "";
        string currFreq = "";
        string OffsetPosition = "0";
        public N5182B(Form5GAT mMainForm)
        {
            mainForm = mMainForm;
        }
        Ivi.Visa.Interop.ResourceManager rm = new Ivi.Visa.Interop.ResourceManager();
        Ivi.Visa.Interop.FormattedIO488 N5182b = new Ivi.Visa.Interop.FormattedIO488();

        LogAdapter log = LogAdapter.Instance;
        public void Log(string text, LogLevel logLevel = LogLevel.INFO)
        {
            log.Log(text, logLevel, Name, "N5182B");
        }
        ...
        //// TODO: Define a new api() using for N7631C.
        private Keysight.SignalStudio.N7631.Api api;
        public void Connection(string IP, string Cmd, out string Manufacturer, out string Model, out string Serial)
        {
            ...
            Model = "ERROR";
            Serial = "ERROR";
            string ConnectCmd = "TCPIP0::" + IP + "::INSTR";
            api = new Api();
            api.New();
            try
            {
                N5182b.IO = (Ivi.Visa.Interop.IMessage)rm.Open("TCPIP::" + IP + "::INSTR", Ivi.Visa.Interop.AccessMode.NO_LOCK, 5, " ");
                N5182b.WriteString(Cmd, true);
                ...
                if (api.ConnectInstrument(ConnectCmd))
                {
                    ...
                    Log("Connection: SUCCESS!(N7631C API)", LogLevel.SUCCESS);
                    Name = $"N5182B";
                }
                else
                {
        ...
        public void Disconnect()
        {
            ...
            {
                //api.Close();
                Log("Connection => CLOSE! (Preset)", LogLevel.WARN);
                api.Preset();
                //N5182b.IO.Close();
            }
            catch (Exception e)
        ...
        public void SendCmd(string Cmd, out string respond)
        {
            ...
            try
            {
                N5182b.WriteString(Cmd, true);
                object[] tmp;
                tmp = (object[])N5182b.ReadList(Ivi.Visa.Interop.IEEEASCIIType.ASCIIType_Any, ",");
        ...
        public void SetFreq(string freqSet)
        {
            try
            {
                ...
                if (mainForm.measurementBackgroundWorker.CancellationPending) return;
                Log($"Set frequency: {double.Parse(freqSet) / 1e6} MHz");
            ...
            {
                Log("Set frequency => FAIL!", LogLevel.ERROR);
            }
        }
        public void SetPower(string setPower)
        {
            try
            {
                ...
                if (mainForm.measurementBackgroundWorker.CancellationPending) return;
                Log($"Set power: {setPower} dBm");
                api.SignalGenerator.Amplitude = double.Parse(setPower);
                api.UpdateToInstrument();
            }
            catch (Exception)
            {
                Log("Set power => FAILED!", LogLevel.ERROR);
            }
        }
        ...
        //        {
        //            Log($"ERROR: {Alias} load Fail! {err}");
        //        }
        //        else
        //        {
        //            Log($"{Alias}: Open file {currLoadPath}");
        //            api.SignalGenerator.Frequency = double.Parse(freqSet);
        //            api.Generate();
        //            api.Download();
        //            currLoadPath = positionCmd;
        //        }
        //    }
        //    catch
        //    {
        ...
        public void LoadGFR1A15(string freqSet, string NRULSCHRBOffset = "0")
        {
            string positionCmd = "C:\\test3gpp\\waveforms\\G-FRC-A1-5.scp";
            if(positionCmd != currLoadPath || freqSet!=currFreq || NRULSCHRBOffset != OffsetPosition)
            {
                try
                {
                    if (mainForm.measurementBackgroundWorker.CancellationPending) return;
                    System.Threading.Thread.Sleep(100);
                    Log($"Loaded file G-FRC-A1-5.scp {positionCmd}");
                    api.OpenSetupFile(positionCmd);
                    string err = api.ReadError();
                    if (!String.IsNullOrEmpty(err))
                    {
                    ...
                    {
                        Log($"Load file FRC at frequency {freqSet} Hz");
                        api.SignalGenerator.Frequency = double.Parse(freqSet);
                        ...
                        currLoadPath = positionCmd;
                        currFreq = freqSet;
                        OffsetPosition = NRULSCHRBOffset;
                    }
                }
                catch
                {
                    Log($"Loaded file {currLoadPath}");
                }
            }
            else
            {
                Log($"Loaded file {positionCmd}");
            }
        ...
        public void SetAWGNSignal()
        {
            try
            {
                if (mainForm.measurementBackgroundWorker.CancellationPending) return;
                System.Threading.Thread.Sleep(100);
                N5182b.WriteString(":RAD:ARB:WAV " + "\"" + "FRC-A2-5-51rb_DDDSU.wfm" + "\"");
                Log("N5182B: Load file: FRC-A2-5-51rb_DDDSU.wfm");
                N5182b.WriteString(":SOUR:RAD:ARB:STAT ON");
            }
            catch (Exception)
            {
                Log("ERROR: N5182B load file fail", LogLevel.ERROR);
            }
        }
        ...
        public void LoadGRFA25(string freqSet, string NRULSCHRBOffset = "0")
        {
            string positionCmd = "C:\\test3gpp\\waveforms\\G-FRC-A2-5.scp";
            if(positionCmd!=currLoadPath || freqSet!=currFreq)
            {
                try
                {
                    if (mainForm.measurementBackgroundWorker.CancellationPending) return;
                    ...
                    if (!String.IsNullOrEmpty(err))
                    {
                        Log($"ERROR: {err}", LogLevel.ERROR);
                    }
                    else
                    {
                        Log($"{Alias}: N5182B load file G-FRC-A2-5.scp");
                        EnableAWGNSignal(true, "10.0", "18360000", "98280000");
                        Log($"{Alias}: N5182B set FRC at frequency {freqSet} Hz");
                        api.SignalGenerator.Frequency = double.Parse(freqSet);
                        Log($"{Alias}: FRC at RB Offset {NRULSCHRBOffset}");
                        SetRBOffset(NRULSCHRBOffset);
                        api.Generate();
                        api.Download();
                    }
            ...
            {
                Log($"{Alias}: N5182B open file G-FRC-A2-5.scp");
            }
        ...
        public void SetRBOffset(string NRULSCHRBOffset)
        {
            try
            {
                if (mainForm.measurementBackgroundWorker.CancellationPending) return;
                System.Threading.Thread.Sleep(100);
                Log($"{Alias}: N5182B setup RB offset {NRULSCHRBOffset} for ULSCH channel");
        ...
        public void EnableAWGNSignal(bool AwgnEnable, string CarriertoNoiseRatio, string CarrierBandwidth, string NoiseBandwidth)
        {
            try
            {
                if (mainForm.measurementBackgroundWorker.CancellationPending) return;
                System.Threading.Thread.Sleep(100);
                Log($"{Alias}: N5180B enables AWGN signal.");
                api.NR5GWaveformSettings.AwgnEnabled = AwgnEnable;
                Log($"{Alias}: N5182B setups carrier to noise ratio {CarriertoNoiseRatio} dB.");
                api.NR5GWaveformSettings.CarrierToNoiseRatio = double.Parse(CarriertoNoiseRatio);
                Log($"{Alias}: N5182B setups carrier bandwidth {CarrierBandwidth} Hz.");
                api.NR5GWaveformSettings.CarrierBandwidth = double.Parse(CarrierBandwidth);
                Log($"{Alias}: N5182B setups noise bandwidth {NoiseBandwidth} Hz.");
                api.NR5GWaveformSettings.NoiseBandwidth = double.Parse(NoiseBandwidth);
            }
            catch (Exception)
            {
                Log("ERROR: N5182B setups AWGN fail", LogLevel.ERROR);
                throw;
            }
        }
        ...
        public void SetICSInterferenceSignal(string freqSet, string RBOffset = "0")
        {
            string positionCmd = "C:\\test3gpp\\waveforms\\ICS_DFT-s-OFDM_50RB.scp";
        ...
        {
            string positionCmd = "C:\\test3gpp\\waveforms\\CW_signal.scp";
            try
            {
                if (mainForm.measurementBackgroundWorker.CancellationPending) return;
                api.OpenSetupFile(positionCmd);
                ...
                {
                    Log("LOG: N5182B load CW_signal successful");
                    api.Generate();
                    api.Download();
                }
            }
            ...
            {
                Log("ERROR: N5182B load CW_signal fail", LogLevel.ERROR);
            }
        }
        ...
        public void NarrowBandUpperIMDSignal(string RBOffset)
        {
        ...
        public void NarrowBandLowerIMDSignal(string RBOffset)
        {
            string positionCmd = "C:\\test3gpp\\waveforms\\IMD_NAB_DFT-s-OFDM_1RB.scp";
            try
            {
        ...
        public void TXIMDSignal()
        {
            ...
            {
                if (mainForm.measurementBackgroundWorker.CancellationPending) return;
                Log($"LOG: N5182B set TX IMD NRTM1.1 10 MHz");
                api.OpenSetupFile(positionCmd);
                string err = api.ReadError();
                if (!String.IsNullOrEmpty(err))
                {
                ...
                {
                    Log("LOG: N5182B load file NRTM11_10MHz_FDD.scp");
                    api.Generate();
                    api.Download();
                }
            }
        ...
        public void FR1_NRTM11_FDD(string freqSet)
        {
            ...
            {
                if (mainForm.measurementBackgroundWorker.CancellationPending) return;
                Log($"LOG: N5182B load NR_TM1.1 signal");
                api.OpenSetupFile(positionCmd);
                string err = api.ReadError();
                if (!String.IsNullOrEmpty(err))
                {
                ...
                {
                    //Log($"LOG: N5182B setups frequency {freqSet} Hz");
                    //api.SignalGenerator.Frequency = double.Parse(freqSet);
                    Log("LOG: N5182B load file NRTM11_FDD.scp");
                    api.Generate();
                    api.Download();
                }
            }
