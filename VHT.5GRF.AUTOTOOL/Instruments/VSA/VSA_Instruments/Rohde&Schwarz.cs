...
namespace _5GAutoTool.Instruments.VSA.VSA_Instruments
{
    public class RohdeSchwarz
    {
        IPAddress ipa;
        string manu;
        string name;
        ...
        Ivi.Visa.Interop.FormattedIO488 rs = new Ivi.Visa.Interop.FormattedIO488();
        bool bLineFeed = true;
        bool bCarriageReturn = true;

        public Form5GAT mainForm;
        public RohdeSchwarz(Form5GAT mMainForm)
        {
            mainForm = mMainForm;
        }
        ...
        public void Connection(string IP, string Cmd, string Instrument, out string manufacturer, out string model, out string serial)
        {
            ...
            try
            {
                rs.IO = (Ivi.Visa.Interop.IMessage)rm.Open("TCPIP::" + IP + "::INSTR", Ivi.Visa.Interop.AccessMode.NO_LOCK, 5, " ");
                rs.IO.Timeout = 2000;
                rs.WriteString(Cmd, true);
                object[] tmp;
                tmp = (object[])rs.ReadList(Ivi.Visa.Interop.IEEEASCIIType.ASCIIType_Any, ",");
                manufacturer = tmp[0].ToString();
                model = tmp[1].ToString();
                serial = tmp[2].ToString();
                if (model == Instrument)
                {
                    Log($"[{manufacturer}]_[{model}]: connected to " + IP, LogLevel.SUCCESS);
                    //Preset configuration
                    rs.WriteString("HCOP:DEV:COL ON"); //turn color printing on_use to save screenshot
                    rs.WriteString("HCOP:DEV:LANG PNG");//file format for  a print job_use to save screenshot
                }

            }
            catch (Exception ex)
            {
                Log($"[ERROR] [{Instrument}] connection fail: {ex}", LogLevel.ERROR);
            }
        }
